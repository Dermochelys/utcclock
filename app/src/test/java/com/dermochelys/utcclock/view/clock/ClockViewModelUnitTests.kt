...
class ClockViewModelUnitTests {
    ...
    private suspend fun verifyRotation(currentPositionLambda: () -> Position) {
        ...
        assertEquals(
            originalPosition,
            currentPositionLambda.invoke(),
            "Did not return to original position"
        )
    }

    private suspend fun advanceTimeAndVerifyNextPosition(currentPositionLambda: () -> Position) {
